...
def sync_clone(tag: str) -> None:
    if not (CLONE_DIR / ".git").exists():
        ...
        run(["git", "clone", "--depth", "1", "--branch", tag, UPSTREAM_CLONE_URL, str(CLONE_DIR)])
        return
...
def copy_upstream_license() -> None:
    ...
    if license_src.exists():
        ...
        print("WARNING: upstream LICENSE.txt not found, keeping existing assets copy", file=sys.stderr)
